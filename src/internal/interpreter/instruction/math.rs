...
impl Interpreter {
    pub fn add(&mut self, dst: &Register, reg_1: &Register, reg_2: &Register) {
        ...
        let dst_value: Constant = constant_1.add(&constant_2);
        ...
        match *dst_loc {
            ...
            RegisterLocation::Global => self.global[dst_index_usize] = Some(dst_value),
            RegisterLocation::Local => {
                let stack_frame = self.ref_stack_frame();
                stack_frame.registers[dst_index_usize] = Some(dst_value);
            }
        }
    }
}