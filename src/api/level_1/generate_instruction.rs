...
impl CodeBuilder<'_> {
    pub fn generate_alloc(&mut self, block_count: u32) {
        self.code_holder.instructions.push(Some(Instruction::Alloc(block_count)));
    }
    pub fn generate_free(&mut self, block_count: u32) {
        self.code_holder.instructions.push(Some(Instruction::Free(block_count)));
    }
    pub fn generate_jump(&mut self, operation_count: i64) {
        self.code_holder.instructions.push(Some(Instruction::Jump(operation_count)));
    }
    pub fn generate_call(&mut self, function_addr: &String) {
        ...
        self.code_holder.instructions.push(Some(Instruction::Call(address)));
    }
}
